...
export const sendMessage = async (text: string): Promise<LexResponse> => {
  ...
  // Special hardcoded query for hotel with Google reviews over 4 stars near Princess Street
  if (
    (lowerText.includes('hotel') || lowerText.includes('accommodation')) &&
    (lowerText.includes('review') || lowerText.includes('rating') || lowerText.includes('star') || lowerText.includes('4')) &&
    (lowerText.includes('princess') || lowerText.includes('downtown'))
  ) {
    return {
      message:
        "Based on Google reviews, the Royal Canadian Legion Villa on Princess Street has over 4 stars. Let me show you its location and details.",
      intent: 'ShowService',
      serviceId: 'poi_100596',
      slots: { category: 'housing' },
    };
  }
  ...
  if (lowerText.includes('grocery') || lowerText.includes('food')) {
    return {
      message:
        "I found several grocery stores nearby. Would you like me to show them on the map?",
      intent: 'FindService',
  ...
  if (lowerText.includes('pharmacy') || lowerText.includes('drugstore') || lowerText.includes('clinic') || lowerText.includes('doctor')) {
    return {
      message:
        "I found several clinics nearby. Would you like me to show them on the map?",
      intent: 'FindService',
